...
\begin{definicion}
	...
	\end{equation*}
	se dice que es exacta si $\Ker(f_{i+1})=\Image(f_i)$ para todo $i$.
...
El interés está en clasificar todas las extensiones de $Q$ por $N$. La equivalencia de extensiones captura la idea de que los grupos centrales se construyen a partir de $N$ y $Q$ esencialmente de la misma forma, ya que $N$ y $Q$ se incluyen y proyectan de la misma manera.
...
\begin{ejemplo}
	El grupo alternado $A_n$ se define como el kernel del homomorfimo del signo $\homo {\sgn} {S_n} {\{1,-1\}^\times}$ que manda una permutación a $1$ si es producto de un número par de transposiciones, y a $-1$ si es un número impar. Por tanto, $S_n$ es una extensión de $C_2$ por $A_n$ dada por $\extension i {\sgn} {A_n} {S_n} {C_2}$.
...
\begin{proposicion}\label{prop:eqiso}Dos extensiones equivalentes $\extension {i_{j}} {\pi_j} N {E_j} Q$ para $j=1,2$, dan lugar a grupos centrales isomorfos.
	\begin{demostracion}
...
El siguiente ejemplo muestra que la equivalencia de extensiones es más fuerte que ser un isomorfismo. 
% ejemplo, poner dem en apendice?
\begin{ejemplo}
	Las dos extensiones siguientes de $\Z_3$ por $\Z_3$ no equivalentes son isomorfas a $\Z_9$.
...
\end{tikzcd}\]
	\begin{demostracion} %$\Z_9$ tiene un único subgrupo de orden $3$ y por tanto la inclusión es única. Para la proyección, se debe mandar el $\bar1$ de $\Z_9$ al $\bar1$ o $\bar2$ de $\Z_3$, lo que da lugar a las dos extensiones mostradas.
		...
		Un automorfismo $f$ de $\Z_9$ viene dado por $f(x) = kx$ con $x\in \Z_9$ y $k\in \Z_9^{\times} = \{1,2,4,5,7,8\}$		 
...
\begin{proposicion}
	...
	\begin{demostracion}
		\begin{enumerate}
			...
			\item Simétrica: Si $\homo f {E_1} {E_2}$ es una equivalencia, por la Proposición \eqref{prop:eqiso}, $\homo {f^{-1}} {E_2} {E_1}$ es una equivalencia. 
...
\begin{observacion}
	Dos extensiones $E$ y $E'$ equivalentes dan lugar a un mismo kernel abstracto. Por ello, para estudiar las extensiones salvo equivalencia podemos fijar una acción $\homo \varphi Q {\Aut(N)}$ y estudiar las extensiones que dan lugar a esa acción.
	\begin{demostracion}
...
\begin{definicion}
	Denotaremos por $\Ext_\varphi(Q,N)$ al conjunto de clases de extensiones equivalentes que dan lugar a la acción externa $\varphi$ de $Q$ en $N$.
\end{definicion}
